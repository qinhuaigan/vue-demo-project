<template lang="html">
  <ul class="clearfix">
    <li class="searchItem" v-for="(item, index) in searchDatas" :key="`${item.placeholder}_${index}`">
      ...
      <el-select filterable :style="{width: item.width}" v-else-if="item.type == 'select'" @change="change(index)"
      ...
      <el-date-picker v-model="models[index]" v-else-if="item.type == 'year'" type="year" @change="change(index)"
